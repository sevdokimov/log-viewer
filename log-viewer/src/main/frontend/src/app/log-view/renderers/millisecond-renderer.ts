...
export class MillisecondRenderer implements TextRenderer {
    ...
    private static exceptionRegexp: RegExp = new RegExp(/\b(\d{4,11}) ?ms\b/ig);
    ...
    tryRender(s: string): Highlight[] {
        MillisecondRenderer.exceptionRegexp.lastIndex = 0;

        let res: Highlight[] = [];

        while (true) {
            let matcher = MillisecondRenderer.exceptionRegexp.exec(s);
            if (!matcher) {
                return res;
            }
            ...
            res.push({start: matcher.index, end: MillisecondRenderer.exceptionRegexp.lastIndex, e});
        }
    }
}
