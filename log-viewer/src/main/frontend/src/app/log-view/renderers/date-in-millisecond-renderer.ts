...
export class DateInMillisecondRenderer implements TextRenderer {
    ...
    private static exceptionRegexp: RegExp = new RegExp(/\b[12]\d{12}\b/ig);
    ...
    tryRender(s: string): Highlight[] {
        DateInMillisecondRenderer.exceptionRegexp.lastIndex = 0;

        let res: Highlight[] = [];

        while (true) {
            let matcher = DateInMillisecondRenderer.exceptionRegexp.exec(s);
            if (!matcher) {
                return res;
            }
            ...
            res.push({start: matcher.index, end: DateInMillisecondRenderer.exceptionRegexp.lastIndex, e});
        }
    }
}
